...
const SPREADSHEET_ID = 'id get from sheet url';
...
function registerForExam(studentId, examType, examYear, subjects, feeStatus, specialPermission) {
  ...
  try {
    const doc = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = doc.getSheetByName('ExamRegistrations');
    if (!sheet) {
      sheet = doc.insertSheet('ExamRegistrations');
      sheet.getRange(1,1,1,12).setValues([['RegistrationID','StudentID','StudentName','Class','Section','ExamType','ExamYear','Subjects','FeeStatus','PermissionBy','RegistrationDate','Status']]);
      sheet.setFrozenRows(1);
    }
    
    const regId = 'EXR-' + Utilities.getUuid().substring(0,8).toUpperCase();
    const student = getStudentById(studentId);
    if (!student) return { success: false, message: 'Student not found' };
    
    sheet.appendRow([
      regId, studentId, student.FullName, student.Class, student.Section,
      examType, examYear, Array.isArray(subjects) ? subjects.join(', ') : subjects,
      feeStatus, specialPermission || '', new Date(), 'Registered'
    ]);
...
function getExamRegistrations(examType, examYear, className) {
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('ExamRegistrations');
    if (!sheet) return [];
    const data = sheet.getDataRange().getValues();
    if (data.length < 2) return [];
    const headers = data[0];
    ...
    if (examType) result = result.filter(r => r.ExamType === examType);
    if (examYear) result = result.filter(r => r.ExamYear === examYear);
    if (className) result = result.filter(r => r.Class === className);
    
    return result;
  } catch(e) { return []; }
}

function updateExamRegistration(regId, updates) {
  const lock = LockService.getScriptLock();
  lock.waitLock(5000);
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('ExamRegistrations');
...
function deleteExamRegistration(regId) {
  ...
  try {
    ...
    const data = sheet.getDataRange().getValues();
    for (let i = data.length-1; i >= 1; i--) {
      if (data[i][0] === regId) { sheet.deleteRow(i+1); return { success: true }; }
    }
    return { success: false };
  } catch(e) { return { success: false }; }
  finally { lock.releaseLock(); }
...
function getExamTypes() {
  return ['Annual Examination', 'Supplementary Examination', 'Mid-Term Examination', 'Term Examination'];
}

function getDefaultSubjects() {
  return ['English', 'Urdu', 'Mathematics', 'Science', 'Islamiyat', 'Social Studies', 'Computer Science', 'Physics', 'Chemistry', 'Biology', 'History', 'Geography', 'Civics', 'Economics', 'Art & Drawing', 'Physical Education'];
}
...
function saveResultCard(data) {
  ...
  try {
    const doc = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = doc.getSheetByName('ResultCards');
    if (!sheet) {
      sheet = doc.insertSheet('ResultCards');
      sheet.getRange(1,1,1,15).setValues([['ResultID','StudentID','StudentName','Class','Section','ExamType','ExamYear','Subjects','Marks','TotalMarks','Percentage','Grade','Remarks','Date','ImageURL']]);
      sheet.setFrozenRows(1);
    }
    
    const resId = 'RES-' + Utilities.getUuid().substring(0,8).toUpperCase();
    const student = getStudentById(data.studentId);
    
    sheet.appendRow([
      resId, data.studentId, student ? student.FullName : '', student ? student.Class : '',
      student ? student.Section : '', data.examType, data.examYear,
      JSON.stringify(data.subjects || []), JSON.stringify(data.marks || {}),
      data.totalMarks || 0, data.percentage || 0, data.grade || '',
      data.remarks || '', new Date(), data.imageUrl || ''
    ]);
...
function getResultCards(studentId, examType, examYear, className) {
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('ResultCards');
    if (!sheet) return [];
    const data = sheet.getDataRange().getValues();
    if (data.length < 2) return [];
    const headers = data[0];
...
function updateResultCard(resId, updates) {
  const lock = LockService.getScriptLock();
  lock.waitLock(5000);
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('ResultCards');
...
function deleteResultCard(resId) {
  ...
  try {
    ...
    const data = sheet.getDataRange().getValues();
    for (let i = data.length-1; i >= 1; i--) {
      if (data[i][0] === resId) { sheet.deleteRow(i+1); return { success: true }; }
    }
    return { success: false };
  } catch(e) { return { success: false }; }
  finally { lock.releaseLock(); }
...
function saveTestResult(data) {
  ...
  try {
    const doc = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = doc.getSheetByName('TestResults');
    if (!sheet) {
      sheet = doc.insertSheet('TestResults');
      sheet.getRange(1,1,1,12).setValues([['TestID','StudentID','StudentName','Class','Section','TestType','Subject','Marks','TotalMarks','Percentage','Date','Remarks']]);
      sheet.setFrozenRows(1);
    }
    
    const testId = 'TST-' + Utilities.getUuid().substring(0,8).toUpperCase();
    const student = getStudentById(data.studentId);
    
    sheet.appendRow([
      testId, data.studentId, student ? student.FullName : '', student ? student.Class : '',
      student ? student.Section : '', data.testType, data.subject,
      data.marks || 0, data.totalMarks || 0, data.percentage || 0,
      data.date || new Date(), data.remarks || ''
    ]);
...
function getTestResults(studentId, testType, subject, className) {
  try {
    const sheet = SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('TestResults');
    if (!sheet) return [];
    const data = sheet.getDataRange().getValues();
    if (data.length < 2) return [];
    const headers = data[0];
    ...
    if (studentId) result = result.filter(r => r.StudentID === studentId);
    if (testType) result = result.filter(r => r.TestType === testType);
...
function deleteTestResult(testId) {
  ...
  try {
    ...
    const data = sheet.getDataRange().getValues();
    for (let i = data.length-1; i >= 1; i--) {
      if (data[i][0] === testId) { sheet.deleteRow(i+1); return { success: true }; }
    }
    return { success: false };
  } catch(e) { return { success: false }; }
  finally { lock.releaseLock(); }
...
function getTestTypes() {
  return ['Weekly Test', 'Monthly Test', 'Quarterly Test', 'Half-Yearly Test', 'Pre-Board Test'];
}
...
function exportAllDataToCSV() {
  try {
    const doc = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheets = ['Students', 'Teachers', 'FeeHistory', 'SalaryHistory', 'Expenses', 'ExamRegistrations', 'ResultCards', 'TestResults'];
    let csvContent = '';
...
function getCSVSheets() {
  try {
    const doc = SpreadsheetApp.openById(SPREADSHEET_ID);
    return doc.getSheets().map(s => s.getName());
  } catch(e) { return []; }
}
...
function deleteDataByYear(sheetName, year) {
  ...
  try {
    ...
    const data = sheet.getDataRange().getValues();
    const headers = data[0];
    const dateCols = ['JoiningDate', 'Date', 'AdmissionDate', 'RegistrationDate'];
    
    let dateColIndex = -1;
    dateCols.forEach(dc => {
      const idx = headers.indexOf(dc);
    ...
    for (let i = data.length - 1; i >= 1; i--) {
      const d = new Date(data[i][dateColIndex]);
